	...
	char* buffer)
{
	...
	for (decltype(model_count) i = 0; i < model_count; ++i)
	{
		...
		ghoul2[i].sg_import(saved_game, ::ri.saved_game->version());
		...
		{
			ghoul2[i].mSlist[x].sg_import(saved_game, ::ri.saved_game->version());
		}
		...
		{
			ghoul2[i].mBlist[x].sg_import(saved_game, ::ri.saved_game->version());
		}
		...
		{
			ghoul2[i].mBltlist[x].sg_import(saved_game, ::ri.saved_game->version());
		}
	}
