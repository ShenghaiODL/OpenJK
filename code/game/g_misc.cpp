...
void shield_power_converter_use( gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	if ( self->setTime < level.time )
	{
		...
		dif = activator->client->ps.stats[STAT_MAX_ARMOR] - activator->client->ps.stats[STAT_ARMOR];
		...
		else if ( activator->client->ps.stats[STAT_ARMOR] >= activator->client->ps.stats[STAT_MAX_ARMOR] )
		{
			// play full sound
